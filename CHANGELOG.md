...
- **Extracted Shared Fast-Listener Unregister Helper:** `sensor.py` removed the fast-update listener in two places — the config-changed branch of `_update_fast_listener_registration()` and `_cleanup_fast_listener()` — with the same try/except/finally block duplicated in both. Extracted into `_unregister_fast_listener()`; the `_is_active` teardown handling stays in `_cleanup_fast_listener()`, which was the only real difference between the two sites.
