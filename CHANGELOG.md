...
- **Number and Time Inputs Keep Following the Inverter:** Those same entities read the hub cache only once, when they were added, and then froze — a charge slot changed on the inverter itself, or a write that the device rejected or clamped, was never reflected back and the UI kept showing a value the inverter did not have. They are now attached to the coordinator and adopt the value of every poll. Setting a value from the UI still applies optimistically and is corrected by the next poll if the inverter disagrees. They deliberately stay available during a failed poll cycle so settings can still be entered.
